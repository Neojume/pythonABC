...
def KRS_ABC(problem, num_samples, epsilon, ksi, S0, delta_S, verbose=False):
    ...
    for i in xrange(num_samples):
        ...
        while True:
            ...
            mu_bar, std, _, N = kernel_regression(theta, xs, ts, h)
            mu_bar_p, std_p, _, N_p = kernel_regression(theta_p, xs, ts, h)
            ...
            for m in xrange(M):
                other_term = \
                    distr.normal.logpdf(y_star, mu_p[m], std_p + (epsilon ** 2)) - \
                    distr.normal.logpdf(y_star, mu[m], std + (epsilon ** 2))
            ...
            if error > ksi:
                ...
                #new_x = distr.uniform.rvs(min(theta, theta_p), max(theta, theta_p))
...
if __name__ == '__main__':
    ...
    samples, rate, sim_calls = KRS_ABC(problem, 1000, 0.0, 0.1, 20, 10)
